...
struct ProviderConnectionTests {
    ...
    static func main() throws {
        ...
        expect(store.provider == .claude, "new install starts on Claude")
        expect(store.selected == [.claude], "selected exposes the single provider as a one-element list")
        store.select(.codex)
        expect(store.provider == .codex && store.left == .codex, "select replaces the single provider")
        expect(store.codexVisible && !store.claudeVisible, "visibility flags follow the selection")
        store.select(.codex)
        expect(store.provider == .codex, "re-selecting the current provider is a no-op")
        store.select(.grok)
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .grok, "selection survives relaunch")
        for provider in IslandProvider.allCases {
            store.select(provider)
            expect(store.provider == provider && store.selected == [provider],
                   "valid selection for \(provider)")
        }
        ...
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .codex, "migrates Codex-only preference to the single slot")
        defaults.removeObject(forKey: ProviderVisibilityStore.selectionKey)
        defaults.set(false, forKey: "MacIsland.codexVisible")
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .claude, "migrates both hidden to Claude")
        defaults.set(["grok", "antigravity"], forKey: ProviderVisibilityStore.selectionKey)
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .grok, "a saved two-provider preference degrades to its left entry")
        expect(defaults.stringArray(forKey: ProviderVisibilityStore.selectionKey) == ["grok"],
               "migration rewrites the key in single-provider shape")
        defaults.set(["unknown", "codex"], forKey: ProviderVisibilityStore.selectionKey)
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .codex, "an unrecognized leading entry falls through to the next")
        defaults.set(["unknown"], forKey: ProviderVisibilityStore.selectionKey)
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.provider == .claude, "a fully unrecognized preference falls back to Claude")
